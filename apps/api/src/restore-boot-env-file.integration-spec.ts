...
// restore-main.ts는 main.ts와 같은 이유로 별도 프로세스(`node dist/restore-main.js`)를
// 띄워야 검증할 수 있다 — main-boot-env-file.integration-spec.ts 참고. gc-main.ts와
// 같은 CLI형 진입점이라 특정 로그 문구를 기다리지 않고 프로세스가 스스로 종료할
// 때까지 기다린다.
describe('restore-main.ts 부팅 순서 (.env 파일 전용 드라이버 설정)', () => {
  let workDir: string;
  ...
  beforeEach(() => {
    workDir = mkdtempSync(path.join(tmpdir(), 'storix-restore-boot-env-'));
  });
  ...
  it('STORIX_DB_DRIVER를 쉘에 export하지 않고 .env 파일에만 설정해도 DataSource가 정상 초기화된다', async () => {
    ...
    writeFileSync(
      ...
      [
        ...
        `STORIX_RESTORE_SOURCE_DIR=${path.join(workDir, 'restore-src')}`,
    ...
    // 복원 대상 파일이 실제로 없어 RestoreJob.run()은 ENOENT로 실패하지만,
    // 그건 DB 계층을 통과했다는 증거다 — DataTypeNotSupportedError만 없으면 된다.
    const { output } = await runProcess({
      cwd: workDir,
      distFile: 'restore-main.js',
