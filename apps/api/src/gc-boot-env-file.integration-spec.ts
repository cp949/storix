...
// main.ts와 달리 계속 떠 있는 서버가 아니라 한 번 실행하고 끝나는 CLI라, 특정
// 로그 문구를 기다리지 않고 프로세스가 스스로 종료할 때까지 기다린다.
describe('gc-main.ts 부팅 순서 (.env 파일 전용 드라이버 설정)', () => {
  let workDir: string;
  ...
  beforeEach(() => {
    workDir = mkdtempSync(path.join(tmpdir(), 'storix-gc-boot-env-'));
  });
