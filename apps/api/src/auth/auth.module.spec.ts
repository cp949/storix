...
describe('resolveValidApiKeys', () => {
  ...
  it('현재 키가 빈 문자열이면 에러를 던진다', () => {
    expect(() => resolveValidApiKeys('', undefined)).toThrow('API_KEY는 비어 있을 수 없다');
  });
...
describe('AuthModule', () => {
  it('API_KEY가 유효하면 VALID_API_KEYS를 정상적으로 조립한다', async () => {
    ...
    const moduleRef = await Test.createTestingModule({ imports: [ConfigModule.forRoot({ isGlobal: true }), AuthModule] })
      ...
      .useValue({
        getOrThrow: (key: string) => (key === 'API_KEY' ? 'a'.repeat(64) : undefined),
        get: () => undefined,
      })
      .compile();
