...
**호출자 (Caller)**:
Storix에 요청을 보내는 쪽이 감사 로그용으로 요청마다 자기신고하는 식별값.
호출 서버가 스스로 붙이는 값이며 Storix는 진위를 검증하지 않는다. 인증(고정
API 키 비교, 통과/실패만 판단)과는 별개 개념이고, 최종 사용자와도 다르다(최종
사용자 식별·인증은 호출 서버 책임이며 Storix 도메인 밖).
_Avoid_: 사용자, 클라이언트, 인증된 주체
