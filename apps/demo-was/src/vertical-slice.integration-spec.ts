...
describe('Demo WAS ↔ 실제 Storix vertical slice', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.DEMO_WAS_NAMESPACE_NAME ??= `demo-it-${Date.now()}`;
    process.env.DEMO_WAS_PUBLIC_NAMESPACE_NAME ??= `demo-it-public-${Date.now()}`;
  ...
  it('업로드 → 인가된 presigned 다운로드 → 공개 발행 → 무인증 다운로드 → 발행 취소 → 404', async () => {
    ...
    await request(app.getHttpServer())
      .put('/demo-api/documents/content?path=/report.txt')
    ...
    const publishResponse = await request(app.getHttpServer())
      .post('/demo-api/documents/publish?path=/report.txt')
      .set('X-Demo-User', 'alice')
      .expect(201);
    ...
    await request(app.getHttpServer())
      .delete('/demo-api/documents/publish?path=/report.txt')
      .set('X-Demo-User', 'alice')
      .expect(204);
