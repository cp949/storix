...
디스크나 NAS 마운트 경로를 S3 API로 노출하고, Storix `app`은 `STORAGE_*` 접속
...
| `STORAGE_ACCESS_KEY` / `STORAGE_SECRET_KEY` | 임의 값 | app 접속 자격증명이자 VersityGW root 자격증명(`ROOT_ACCESS_KEY`/`ROOT_SECRET_KEY`). 운영에서는 기본값을 교체 |
| `STORAGE_BUCKET` | 버킷 이름 | `versitygw-init`이 기동 시 생성 |
| `VERSITYGW_DATA_PATH` | 비움 또는 `/`로 시작하는 절대 경로 | 비우면 named volume `versitygw-data`. NAS면 절대 경로 |
...
| `STORAGE_PUBLIC_ENDPOINT` / `STORAGE_PUBLIC_PORT` / `STORAGE_PUBLIC_USE_SSL` | 클라이언트가 접근 가능한 VersityGW 주소 | presigned download를 쓸 때만. 비우면 그 API만 실패 |
| `STORAGE_REGION` | 예: `us-east-1` | presigned download를 쓸 때 비우면 안 됨(아래 문제 해결) |

override가 덮어써서 무시되는 값: `STORAGE_ENDPOINT` / `STORAGE_PORT` /
`STORAGE_USE_SSL`(`versitygw` / `7070` / `false`로 고정).
...
```bash
STORAGE_ENDPOINT=gw.internal.example
STORAGE_PORT=7070
...
운영에서는 `STORAGE_PUBLIC_*`에 클라이언트가 실제로 접속하는 공개 주소·포트·
...
- **`VERSITYGW_DATA_PATH`는 절대 경로여야 한다.** `/` 없이 쓰면 Docker Compose가
  bind mount가 아니라 named volume 이름으로 조용히 해석해 NAS가 아닌 로컬
  볼륨에 쓴다.
- **멀티 인스턴스(공유 DB + NAS)**: 모든 WAS 호스트가 같은 `VERSITYGW_DATA_PATH`와
  같은 `DB_HOST`를 쓴다. `docker-compose.postgres.yml`은 겹치지 않는다. 여러
...
- **데이터 초기화**:
  ...
  VERSITYGW_DATA_PATH로 bind mount한 NAS 경로는 삭제되지 않는다.
  ```
